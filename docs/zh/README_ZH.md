...
**為什麼用 centroid，而不是 LLM 摘要？** Patrick 的核心約束是零 Token 消耗。呼叫 LLM 來摘要每個 session 會增加延遲、API 成本，並且打破「完全本地」的承諾。因此我們採用純數學方法：一個 session 中所有 chunk 向量的 centroid（均值）是一個非常有效的「主題指紋」——它捕捉了整個討論的加權語義中心，無需理解語言本身。

**運作方式——每個 session 結束時，Patrick：**
...
**搜尋如何使用摘要：**
- 查詢時，第一層將你的查詢做 embedding，然後對 `session_summaries` 中的 centroid 向量做 **cosine 相似度搜尋**——快速定位哪些 session 與查詢語義相關。
- 只有 top-K 命中的 session 進入第二層，在其中搜尋具體的 chunk 取得精確答案。
- 這種兩層設計避免了直接搜尋數百萬個 chunk——centroid 作為廉價的「session 級索引」，將搜尋範圍縮小 10～100 倍。

**Phase 4 補充：** 為了人類可讀的展示，每個 session 還儲存了 `opening`（第一句 user prompt，≤200 字）和 `body`（前 5 個 assistant 回覆，去重後）。這些欄位用於 `memory_sessions` 列表顯示，**不影響搜尋**——搜尋始終使用 centroid 向量。
